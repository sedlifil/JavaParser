...
public class ContainerClassCU {
    ...
    public ContainerClassCU(String nameClassWithAbsPath) throws FileNotFoundException {
        this.nameClassWithAbsPath = nameClassWithAbsPath;
        compilationUnit = JavaParser.parse(new FileInputStream(nameClassWithAbsPath));
        setUpName();
    }
    ...
    public String getBelongToBlocks() {
        if (belongToBlocks == null) {
            belongToBlocks = "";
            List<AnnotationExpr> annotationClassList = getClassAnnotations();
            ...
            for (AnnotationExpr ann : annotationClassList) {
                if (ann.getNameAsString().equals(ParserClass.BLOCK_)) {
                    ...
                    belongToBlocks = nax.getMemberValue().toString();
                }
            }
        }
    ...
    private List<String> addImportsAsClassName(List<ImportDeclaration> imports) {
        ...
        return imports.stream().map(y -> {
            ...
            return import_.substring(import_.indexOf(" ") + 1, import_.lastIndexOf(";"))
                    .replace(".", ParserClass.FILE_DELIMITER);
