...
public class MethodParserTest {
    ...
    public void categorizeMethodsForBlock1() {
        ...
        list1.forEach(y -> assertTrue("container1 neobsahuje metodu: " + y, container1.getMethodDeclarations()
                .stream().map(NodeWithSimpleName::getNameAsString).collect(Collectors.toList()).contains(y)));
        assertEquals("Lists are not equal", list1.size(), container1.getMethodDeclarations().size());

        list2.forEach(y -> assertTrue("container2 neobsahuje metodu: " + y, container2.getMethodDeclarations()
                .stream().map(NodeWithSimpleName::getNameAsString).collect(Collectors.toList()).contains(y)));
        assertEquals("Lists are not equal", list2.size(), container2.getMethodDeclarations().size());
